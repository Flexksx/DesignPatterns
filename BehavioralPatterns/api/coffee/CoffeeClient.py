...
class CoffeeClient():
    ...
    def get(self, milk: MilkType = None, bean: BeanType = None, syrup: SyrupType = None, take_out: bool = False):
        return self._coffee_builder.with_milk(milk).with_bean(bean).with_syrup(syrup).for_take_out(take_out).build()

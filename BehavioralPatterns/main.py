...
another_coffee = (
    CoffeeBuilder()
    .with_milk(MilkType.REGULAR)
    .with_bean(BeanType.ROBUSTA)
    .with_syrup(SyrupType.COCONUT)
    .on_the_place()
    .build()
)
...
order = (
    ...
    .add_pastry(PastryFactory().create_pastry(pastry_type=PastryType.CROISSANT))
    .build()
)
...
barista.handle_request({"request": "order"})
barista.handle_request({"request": "order"})
barista.handle_request({"request": "order"})
barista.handle_request({"request": "order"})
barista.handle_request({"request": "order"})
